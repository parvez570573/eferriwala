...
class Elementor_AjaxLoader {
	...
	public static function get_products_of_category( $settings ) {
        ...
        <div class="cats-info">
            <div class="cats-inner">
                <h2><?php echo self::get_link_control( $settings['c_link'], $settings['title'], 'cat-title' ) ?></h2>
	...
	public static function get_products_of_category_2( $settings ) {
		...
		if ( $settings['title'] ) {
			...
			$cats_html[] = sprintf( '<h2>%s</h2>', self::get_link_control( $settings['link'], $title, 'cat-title' ) );
		}
	...
	public static function get_product_tabs_handler( $settings ) {
		...
		if ( ! empty( $settings['title'] ) ) {
			$header_tabs[] = sprintf( '<h2>%s</h2>', self::get_link_control( $settings['link'], $settings['title'], 'cat-title' ) );
		}
			...
			if ( $cats ) {
				foreach ( $cats as $tab ) {
					...
					if ( ! is_wp_error( $term ) && $term ) {
						$header_tabs[] = sprintf( '<li><a href="#" class="%s" data-href="%s">%s</a></li>', esc_attr( $class_active ), esc_attr( $tab['product_cat'] ),  esc_html( $term->name ) );
					}
					...
					if ( $i == 0 ) {
						$tab_content[] = sprintf( '<div class="tabs-panel tabs-%s tab-loaded active">%s</div>', esc_attr( $tab['product_cat'] ), Elementor::get_products( $tab_atts ) );
					} else {
						if ( $settings['lazy_loading'] == 'yes' ) {
	...
	public static function get_product_grid_handler( $settings ) {
		...
		if ( ! empty( $settings['title'] ) ) {
			$output[] = sprintf( '<h2 class="cat-title">%s</h2>', self::get_link_control( $settings['link'], $settings['title'], '' ) );
		}
